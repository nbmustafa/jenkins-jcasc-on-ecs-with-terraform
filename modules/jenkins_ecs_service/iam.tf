...
data "aws_iam_policy_document" "master_ecs_task" {
  ...
  statement {
    ...
    resources = ["arn:aws:ecs:${local.aws_region}:${data.aws_caller_identity.current.account_id}:task-definition/*"]
  }
  ...
  statement {
    ...
    resources = ["arn:aws:ecs:${local.aws_region}:${data.aws_caller_identity.current.account_id}:task/*"]
  }
}
